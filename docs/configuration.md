    ...
    snapshot: # when mode is snapshot or snapshot_and_replication
      ...
      schema: # when mode is full or schema
        pgdump_pgrestore:
          ...
          index_build_maintenance_work_mem: 1GB # value applied as maintenance_work_mem on the session that restores indices/constraints, to speed up index creation. Unset by default (server default).
          index_build_max_parallel_maintenance_workers: 4 # value applied as max_parallel_maintenance_workers on the session that restores indices/constraints, to allow parallel index builds. Unset by default (server default). Bounded server-side by max_worker_processes.
...
| PGSTREAM_POSTGRES_SNAPSHOT_INDEX_BUILD_MAINTENANCE_WORK_MEM | ""                       | No       | When using `pg_dump`/`pg_restore` to snapshot schema for Postgres targets, value applied as `maintenance_work_mem` (e.g. `1GB`) on the session that restores indices and constraints, to speed up index creation. Empty leaves the server default.                                                            |
| PGSTREAM_POSTGRES_SNAPSHOT_INDEX_BUILD_MAX_PARALLEL_MAINTENANCE_WORKERS | 0            | No       | When using `pg_dump`/`pg_restore` to snapshot schema for Postgres targets, value applied as `max_parallel_maintenance_workers` on the session that restores indices and constraints, to allow parallel index builds. 0 leaves the server default. Bounded server-side by `max_worker_processes`.              |
...
| PGSTREAM_POSTGRES_SNAPSHOT_DISABLE_PROGRESS_TRACKING    | False                        | No       | Whether to disable progress tracking for the snapshot.                                                                                                                                                                                                                                                       |
| PGSTREAM_POSTGRES_LISTENER_EXP_BACKOFF_INITIAL_INTERVAL | 500ms                        | No       | Initial interval for the exponential backoff policy to be applied to the Postgres connection retries.                                                                                                                                                                                                        |
| PGSTREAM_POSTGRES_LISTENER_EXP_BACKOFF_MAX_INTERVAL     | 10s                          | No       | Max interval for the exponential backoff policy to be applied to the Postgres connection retries.                                                                                                                                                                                                            |
...
| PGSTREAM_POSTGRES_WRITER_BATCH_AUTO_TUNE_CONVERGENCE_THRESHOLD | 0.01 (1%)                       | No       | Minimum search space width (as a fraction of the max batch bytes) at which the auto tune process stops searching for a better batch size.                                                                      |
