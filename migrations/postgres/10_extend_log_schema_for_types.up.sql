-- Extend log_schema function to handle CREATE TYPE, ALTER TYPE, and DROP TYPE events
...
BEGIN
    ...
    IF tg_tag = 'DROP SCHEMA' AND tg_event = 'sql_drop' THEN
        ...
        END IF;
    elsif tg_tag = 'DROP TYPE' AND tg_event = 'sql_drop' THEN
        -- Handle DROP TYPE events
        SELECT schema_name INTO rec_schema_name FROM pg_event_trigger_dropped_objects() WHERE object_type = 'type' LIMIT 1;
        ...
        elsif tg_tag = 'CREATE TYPE' THEN
            -- Handle CREATE TYPE events (including enums)
            SELECT schema_name INTO rec_schema_name FROM pg_event_trigger_ddl_commands() WHERE object_type = 'type' AND command_tag = 'CREATE TYPE' LIMIT 1;
        elsif tg_tag = 'ALTER TYPE' THEN
            -- Handle ALTER TYPE events (e.g., adding enum values)
            SELECT schema_name INTO rec_schema_name FROM pg_event_trigger_ddl_commands() WHERE object_type = 'type' AND command_tag = 'ALTER TYPE' LIMIT 1;
        END IF;
